...
Lexer.prototype.token = function(src, top) {
  ...
  block.paragraph = replace(base_paragraph)('fences|list', "")('hr', block.hr)('heading', block.heading)('lheading', block.lheading)('tag', '<' + block._tag)();

  block.blockquote = replace(block.blockquote)('paragraph', block.paragraph)();
  ...
  block.gfm.paragraph = replace(base_paragraph)('fences', block.gfm.fences.source.replace('\\1', '\\2'))('list', block.list.source.replace('\\1', '\\3'))('hr', block.hr)('heading', block.heading)('lheading', block.lheading)('blockquote', block.blockquote)('tag', '<' + block._tag)('def', block.def)();
  ...
  Lexer = function () {
    class Lexer {
      ...
      token(src, top) {
        var b, base1, bull, cap, checkbox, checked, i, item, k, l, len, len1, len2, len3, loose, m, next, o, p, q, ref, ref1, ref2, ref3, ref4, space, tag, type;
        while (src) {
          // newline
          if (cap = this.rules.newline.exec(src)) {
  ...
  inline.gfm.url = replace(inline.gfm.url)('email', inline._email)();
...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    ruby(ruby, title, text) {
      if (title) {
        return `<span class="btn" title="${title}"><ruby>${text}<rp>《</rp><rt>${ruby}</rt><rp>》</rp></ruby></span>`;
      } else {
    ...
    note(num, title) {
      return `<sup title="${title}"><b>${num}</b></sup>`;
    }
...
function marked(src, opt, callback) {
  ...
  Parser = class Parser {
    ...
    parse(src) {
      ...
      if (src.notes.length) {
        ...
        out += this.renderer.list(notes, true);
      }
     ...
     */
    tok() {
      var _row, body, cell, checked, flags, header, html, i, j, k, len, len1, len2, m, o, ordered, p, ref, ref1, row, taskList, tasklist;
      switch (this.token.type) {
        case 'space':
  ...
  marked = function (src, opt, callback) {
    var done, e, highlight, message, pending, tokens, txt;
    // throw error in case of non string input
    if (!src) {
      throw new Error('marked(): input parameter is undefined or null');
    }
