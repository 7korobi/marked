...
(function () {
   ...
   */
  var InlineLexer, Lexer, Parser, Renderer, TextRenderer, baseUrls, block, escape, inline, marked, noop, originIndependentUrl, replace, resolveUrl, splitCells, unescape;
  ...
  block.html = replace(block.html, 'i')('comment', block._comment)('tag', block._tag)('attribute', / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/)();
  ...
  Lexer = function () {
    class Lexer {
      ...
      token(src, top) {
        var b, base1, bull, cap, checkbox, checked, i, is_ordered, item, k, l, len, len1, len2, len3, loose, m, next, o, p, q, ref, ref1, ref2, ref3, ref4, ref5, space, tag, type;
        while (src) {
          // newline
          ...
          if (top && (cap = this.rules.nptable.exec(src))) {
            ...
            item = {
              ...
              cells: ((ref = cap[3]) != null ? ref.replace(/\n$/, '').split('\n') : void 0) || []
            };
            ref1 = item.align;
  ...
  inline.tag = replace(inline.tag)('comment', block._comment)('attribute', inline._attribute)();

  inline.link = replace(inline.link)('label', inline._label)('href', inline._href)('title', inline._title)();

  inline.reflink = replace(inline.reflink)('label', inline._label)();

  inline.note = replace(inline.note)('label', inline._label)();
    ...
    class InlineLexer {
      ...
      output(src) {
        ...
        while (src) {
          ...
          // anker
          if (cap = this.rules.anker.exec(src)) {
            // console.log 'anker', cap
            src = src.slice(cap[0].length);
            out += this.renderer.anker(cap[0].slice(2));
            continue;
          }
      ...
      outputLargeBrackets({ mark, text }, link) {
        ...
        switch (false) {
          ...
          case !/^$|^mailto:|:\/\/|^(\.{0,2})[\?\#\/]|^[\w()%+:\/]+$/ig.exec(href):
